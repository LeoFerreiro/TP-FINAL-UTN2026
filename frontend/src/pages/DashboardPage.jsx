...
export function DashboardPage() {
  ...
  async function saveTask(data) {
    ...
    if (user.demo) setTasks((current) => {
      ...
      const recurrence = data.recurrence?.enabled ? { ...data.recurrence, seriesId: previous?.recurrence?.seriesId || data.recurrence.seriesId || crypto.randomUUID() } : { enabled: false, weekdays: [] };
      const saved = { ...data, recurrence, _id: data._id || crypto.randomUUID(), category: categoryObject };
  ...
  async function cleanupCompleted(ids, all) {
    ...
    try {
      if (user.demo) {
        setTasks((current) => {
          const completed = current.filter((task) => task.status === "completed" && (all || ids.includes(task._id)));
          const seriesIds = completed.map((task) => task.recurrence?.seriesId).filter(Boolean);
          return current.filter((task) => !completed.some((item) => item._id === task._id) && !seriesIds.includes(task.recurrence?.seriesId));
        });
      } else {
        await api("/tasks/cleanup-completed", { method: "POST", body: JSON.stringify({ ids, all }) });
        setTasks(await api("/tasks"));
      }
    } catch (err) {
