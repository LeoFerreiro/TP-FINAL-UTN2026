...
export function RegisterPage() {
  ...
  async function submit(event) {
    ...
    try { const data = await api("/auth/register", { method: "POST", body: JSON.stringify(form) }); setMessage(data.message); } catch (err) { setError(err.message); }
  }
  return <AuthShell eyebrow="Empezá hoy" title="Creá tu cuenta" subtitle="Registrate y validá tu correo para comenzar.">
    {message ? <div className="success-box"><strong>¡Cuenta creada!</strong><p>{message}</p></div> : <form onSubmit={submit} className="auth-form">
